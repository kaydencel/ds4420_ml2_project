---
title: "Airbnb Bayesian Modeling — Interactive"
author: "Ayush Desai"
date: "`r Sys.Date()`"
...
ui <- navbarPage(
  ...
  header = tags$head(
    tags$style(HTML("
      ...
      .irs--shiny .irs-bar,
      .irs--shiny .irs-single { background: #1a1a2e !important; border-color: #1a1a2e !important; }
    ...
    fluidRow(column(10, offset = 1,
      div(class = "overview-section",
        ...
        p("Dynamic pricing adjusts prices in response to market conditions — supply and demand shifts,
          ...
          1980s–90s, it has expanded dramatically through big data and machine learning."),
        p("Today, companies use personal data — browsing behavior, device info, demographics, even battery
          level — to infer each consumer's willingness to pay. This practice, sometimes called ",
          tags$b("surveillance pricing"), " or ", tags$b("predatory pricing"),
          ", raises serious concerns around consumer protection and data privacy. No federal U.S. legislation
          currently restricts it, though Maryland's Predatory Pricing Act (2026) and the EU Omnibus Directive
          (2019/2161) represent early regulatory steps."),
        ...
        p("Ghost listings distort platform supply signals, may manipulate neighborhood pricing benchmarks,
        ...
        tags$ul(
          tags$li(tags$b("Price Model (Gaussian): "),
            ...
            review count, and composite review score. Returns a full posterior predictive distribution."),
        ...
        h4("Data Source"),
        p("Data sourced from ", tags$a("Inside Airbnb", href = "http://insideairbnb.com", target = "_blank"),
          ", a public repository of scraped NYC Airbnb listings. Filtered to prices between $10–$2,000/night.
          Key fields: neighbourhood, room type, accommodates, availability, reviews per month,
          review scores, host listing count."),
  ...
  tabPanel("Ghost Detection",
    sidebarLayout(
      sidebarPanel(width = 3,
        ...
        sliderInput("price_vs_neigh_val", "Price vs. Neigh (log)", min = -2,  max = 2,   value = 0,   step = 0.1),
    ...
    fluidRow(
      column(3,
        div(class = "well",
          ...
          sliderInput("tbl_prob_thresh", "Min Ghost Prob",
...
server <- function(input, output, session) {
  ...
  output$price_dist_plot <- renderPlot({
    ...
    tibble(price = draws) %>%
      ggplot(aes(x = price)) +
      geom_histogram(bins = 60, fill = "#1a1a2e", alpha = 0.85, color = NA) +
      geom_vline(xintercept = median(draws), color = "#c0392b",
                 linewidth = 0.9, linetype = "dashed") +
      scale_x_continuous(labels = scales::dollar, limits = c(0, 800)) +
      labs(subtitle = paste0(input$borough, "  ·  ", input$room_type_price,
                             "  ·  ", input$accommodates, " guests  ·  score: ",
                             input$review_score),
           x = "Predicted Nightly Price (USD)", y = "Posterior Draws") +
      theme_minimal(base_size = 11) +
      theme(
        plot.subtitle    = element_text(size = 9, color = "#666"),
        panel.grid.minor = element_blank(),
        panel.grid.major = element_line(color = "#eeeeee"),
        axis.title       = element_text(size = 10, color = "#444")
      )
  }, res = 96)
  ...
  output$ghost_prob_card <- renderUI({
    ...
    div(
      ...
      p(style = "margin:0; font-size:0.72em; text-transform:uppercase;
                 letter-spacing:0.09em; color:#777;
                 font-family:'Helvetica Neue',Arial,sans-serif;",
