...
```{r load-data}
# Load pre-computed Bayesian model data
...
# Load MLP predictions and join with listing metadata for the Flagged Listings table.
# IDs are read as character strings to avoid precision loss with large integers.
...
listings <- read_csv("../data/airbnb_clean.csv",
                     col_types = cols(id = col_character()),
                     show_col_types = FALSE) %>%
  select(-1) # drop the unnamed pandas row-index column
...
# Builds a named numeric vector matching the price model's coefficient names.
# Treatment contrasts are used so the first level of each factor is the reference.
...
# Builds a named numeric vector matching the ghost model's coefficient names.
...
# Draws from the posterior predictive distribution for price and converts back to dollars.
...
# Returns the posterior mean ghost probability using the logistic link.
  ...
  tabPanel("Overview",
    fluidRow(column(10, offset = 1,
      div(class = "overview-section",
        ...
        p("Three models are used in this application:"),
        tags$ul(
          tags$li(tags$b("Price Model (Bayesian Gaussian): "),
            "Predicts log(nightly price) from neighborhood median price, room type, guest capacity,
            review count, and composite review score. Uses weakly informative priors —
          ...
          tags$li(tags$b("Ghost Model (Bayesian Logistic): "),
            ...
            from neighborhood median, review score, and review count. Same prior specification as
            the price model. Powers the Ghost Detection tab."),
          tags$li(tags$b("Ghost Classifier (MLP): "),
            "A four-layer neural network (128 → 64 → 32 → 1 nodes) with ReLU activations,
            30% dropout, and cosine learning rate decay. Trained on 14,990 listings with the
            ghost label computed directly from the raw listing data. Achieves 82.5% accuracy,
            73.9% recall, 58.1% precision, and 0.876 AUC. Powers the Flagged Listings tab.")
        ),
        p("Both Bayesian models are fit using ", tags$code("brms"),
          " with 4 MCMC chains and 2,000 iterations (1,000 warmup) each."),

        h4("Data Source"),
        p("Data from ", tags$a("Inside Airbnb", href = "http://insideairbnb.com", target = "_blank"),
          ", a public repository of scraped NYC listings. The raw dataset contains 36,353 New York
          City listings from November 2025. After filtering for complete feature data, 21,415
          listings were used for modeling, of which 22% are flagged as ghost listings."),
...
server <- function(input, output, session) {
  ...
  # Price prediction — ignoreNULL = FALSE means results show on load before clicking the button
  ...
  # Ghost prediction — ignoreNULL = FALSE means results show on load
  ...
  # Color-coded risk card: green below 10%, orange 10-30%, red above 30%
    ...
    tryCatch({
      df_ghost %>%
        ...
        summarise(avg_prob = mean(ghost_prob),
                  n_flagged = sum(ghost_pred),
                  .groups = "drop") %>%
        ggplot(aes(x = reorder(neighbourhood_group_cleansed, avg_prob), y = avg_prob)) +
        ...
        labs(x = NULL, y = "Mean MLP Ghost Probability") +
        theme_minimal(base_size = 11) +
        theme(panel.grid.minor = element_blank(),
              panel.grid.major.y = element_blank(),
              panel.grid.major.x = element_line(color = "#eeeeee"),
  ...
  # Filter MLP-flagged listings by probability threshold and borough
